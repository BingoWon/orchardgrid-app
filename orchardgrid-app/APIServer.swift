...
final class APIServer {
    ...
    private nonisolated func handleConnection(_ connection: NWConnection) async {
        ...
        guard let rawRequest = await receiveRequest(from: connection),
              let httpRequest = HTTPRequest(rawRequest: rawRequest) else {
    ...
    private nonisolated func handleChatCompletion(request: HTTPRequest, connection: NWConnection) async {
        ...
        do {
            ...
            let systemPrompt = chatRequest.messages.first(where: { $0.role == "system" })?.content ?? defaultSystemPrompt
            ...
            await sendError(.badRequest, message: "Invalid request format: \(error.localizedDescription)", to: connection)
        }
    }
    ...
    private nonisolated func sendResponse(
        ...
        do {
            ...
            if errorMessage.contains("context") || errorMessage.contains("window") {
                await sendError(.badRequest, message: "Context window exceeded. Please start a new conversation.", to: connection)
            } else {
        ...
        do {
            guard let lastMessage = messages.last, lastMessage.role == "user" else {
                let errorChunk = StreamChunk(
                    ...
                    created: timestamp,
                    model: "apple-intelligence",
                    choices: [.init(index: 0, delta: .init(role: "assistant", content: "Error: Last message must be from user"), finishReason: "error")]
                )
                ...
                for try await snapshot in stream {
                    ...
                    if !delta.isEmpty {
                        let chunk = StreamChunk(
                            ...
                            created: timestamp,
                            model: "apple-intelligence",
                            choices: [.init(index: 0, delta: .init(role: "assistant", content: delta), finishReason: nil)]
                        )
            ...
            let finalChunk = StreamChunk(
                ...
                created: timestamp,
                model: "apple-intelligence",
                choices: [.init(index: 0, delta: .init(role: "assistant", content: ""), finishReason: "stop")]
            )
            ...
            let chunk = StreamChunk(
                ...
                created: timestamp,
                model: "apple-intelligence",
                choices: [.init(index: 0, delta: .init(role: "assistant", content: errorContent), finishReason: "error")]
            )
            await sendStreamChunk(chunk, to: connection)
            await sendStreamEnd(to: connection)
        }
    ...
    private nonisolated func send<T: Encodable>(_ response: T, to connection: NWConnection) async {
    ...
    private nonisolated func sendStreamChunk<T: Encodable>(_ chunk: T, to connection: NWConnection) async {
    ...
    private nonisolated func sendError(_ error: HTTPError, message: String? = nil, to connection: NWConnection) async {
    ...
    private nonisolated func send(_ text: String, to connection: NWConnection, closeAfter: Bool) async {

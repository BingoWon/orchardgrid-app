...
final class WebSocketClient {
    ...
    init(serverURL: String = "wss://orchardgrid-api.bingow.workers.dev/device/connect", deviceID: String = "device-001") {
        ...
        if let responseFormat = request.response_format,
           ...
           let jsonSchema = responseFormat.json_schema {
            ...
            for try await snapshot in stream {
                ...
                print("🌊 [Streaming] Chunk \(chunkCount): delta length = \(delta.count), full length = \(fullContent.count)")
    ...
    private func generateResponse(for request: ChatRequest) async throws -> ChatResponse {
        ...
        return ChatResponse(
            ...
            usage: ChatResponse.Usage(
                ...
                totalTokens: estimateTokens(request.messages) + estimateTokens([ChatMessage(role: "assistant", content: content)])
            )
        )
    }
    ...
    private func sendMessage<T: Encodable>(_ message: T) async {
        do {

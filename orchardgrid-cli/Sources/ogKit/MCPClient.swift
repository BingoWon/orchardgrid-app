...
// MARK: - Lifecycle helper for inference commands
//
// Spin up MCP servers for the duration of `body`, always shutting
// them down afterwards. Refuses `--mcp` with `--host` (MCP tool
// calling requires on-device inference). MCPError thrown by
// MCPManager construction is translated to OGError.runtime so the
// top-level handler in OgMain sees a single error taxonomy.
